...
CREATE TABLE IF NOT EXISTS bookings (
    ...
    extra_schedule_ids INTEGER[] DEFAULT '{}',
    UNIQUE(schedule_id, box_id)
);

-- Add extra_schedule_ids to existing bookings table if not exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name='bookings' AND column_name='extra_schedule_ids'
  ) THEN
    ALTER TABLE bookings ADD COLUMN extra_schedule_ids INTEGER[] DEFAULT '{}';
  END IF;
END $$;
