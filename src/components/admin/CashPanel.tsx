...
export function CashPanel() {
  ...
  const unpaidBookings = bookings?.filter((b) => b.status === 'completed' && !b.is_paid) || [];
  ...
  const payMutation = useMutation({
    ...
    onMutate: (id) => {
      setPayingBookingIds((ids) => ids.includes(id) ? ids : [...ids, id]);
    },
    onSuccess: (_data, id) => {
      queryClient.setQueryData<Booking[]>(['bookings'], (current) =>
        current?.map((booking) => booking.id === id ? { ...booking, is_paid: true } : booking)
      );
      queryClient.invalidateQueries({ queryKey: ['cash'] });
    ...
    onError: (error: any) => {
      toast.error(error?.response?.status === 409 ? 'Эта запись уже оплачена' : 'Ошибка подтверждения оплаты');
    },
    onSettled: (_data, _error, id) => {
      setPayingBookingIds((ids) => ids.filter((bookingId) => bookingId !== id));
    },
  });
  ...
  const pageSize = 25;
  const totalPages = Math.max(1, Math.ceil((operations?.length ?? 0) / pageSize));
  const currentPage = Math.min(page, totalPages);
  const pagedOperations = operations?.slice((currentPage - 1) * pageSize, currentPage * pageSize) ?? [];
    ...
    <div className="space-y-4">
      ...
      {unpaidBookings.length > 0 && (
        <div className="space-y-2 mb-6">
          ...
          {unpaidBookings.map((b) => {
            const isPaying = payingBookingIds.includes(b.id);
                ...
                <CardContent className="p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                    ...
                    <Button
                      ...
                      disabled={isPaying}
                    >
                      {isPaying ? 'Оплата...' : 'Подтвердить оплату'}
                    </Button>
                  </div>
                </CardContent>
              </Card>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-3">
          <h3 className="text-lg font-semibold text-gray-900">Операции</h3>
          <p className="text-sm text-gray-500">{operations?.length ?? 0} чеков</p>
        </div>
